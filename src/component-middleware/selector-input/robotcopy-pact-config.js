...
class SelectorInputMessageHandler {
    ...
    handleIncomingMessage(message, sender, sendResponse) {
        ...
        console.log('🎯 SelectorInput: Received message:', { type, source, target, traceId });
        ...
        if (this.messageHandlers.has(type)) {
            ...
            try {
                ...
                console.error('🎯 SelectorInput: Error handling message:', error);
                sendResponse({ success: false, error: error.message, traceId });
            }
        } else {
            console.warn('🎯 SelectorInput: No handler for message type:', type);
...
const createSelectorInputStateMachine = () => {
    console.log('🎯 SelectorInput: State machine factory called (simplified version)');
    
    // Return a simple mock for now
    return {
        send: async (event) => {
            console.log('🎯 SelectorInput: Mock state machine received event:', event);
            return { success: true };
        },
        getSnapshot: () => ({
            value: 'idle',
