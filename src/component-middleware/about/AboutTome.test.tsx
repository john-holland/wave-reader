...
import AboutTome from './AboutTome';
...
// Mock EditorWrapper to avoid ErrorBoundary import issues
    ...
    default: ({ children, title, description }: any) => {
      return (
        <div data-testid="editor-wrapper">
          ...
          <div data-testid="editor-description">{description}</div>
...
describe('AboutTome - Idle State Rendering', () => {
  ...
  test('idle state renders content when machine is in idle state', async () => {
    ...
    // Verify section headers are present (use getAllByText for multiple matches)
    expect(screen.getAllByText(/About Wave Reader/).length).toBeGreaterThan(0);
